...
func DeletevisitHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletevisitTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_visits_visitId",
		mcp.WithDescription("Delete a visit by ID"),
		mcp.WithNumber("visitId", mcp.Required(), mcp.Description("The ID of the visit.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletevisitHandler(cfg),
	}
}
