...
func GetownerHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetownerTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_owners_ownerId",
		mcp.WithDescription("Get a pet owner by ID"),
		mcp.WithNumber("ownerId", mcp.Required(), mcp.Description("The ID of the pet owner.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetownerHandler(cfg),
	}
}
