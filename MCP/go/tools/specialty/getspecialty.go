...
func GetspecialtyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetspecialtyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_specialties_specialtyId",
		mcp.WithDescription("Get a specialty by ID"),
		mcp.WithNumber("specialtyId", mcp.Required(), mcp.Description("The ID of the speciality.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetspecialtyHandler(cfg),
	}
}
