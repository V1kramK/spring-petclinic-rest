...
func DeletespecialtyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletespecialtyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_specialties_specialtyId",
		mcp.WithDescription("Delete a specialty by ID"),
		mcp.WithNumber("specialtyId", mcp.Required(), mcp.Description("The ID of the specialty.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletespecialtyHandler(cfg),
	}
}
