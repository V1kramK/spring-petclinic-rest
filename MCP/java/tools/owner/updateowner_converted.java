/**
 * MCP Server function for Update a pet owner's details
 */
...
class Put_Owners_Owner_IdMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPut_Owners_Owner_IdHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("ownerId")) {
            queryParams.add("ownerId=" + args.get("ownerId"));
                ...
                String url = config.getBaseUrl() + "/api/v2/put_owners_owner_id" + queryString;
    ...
    public static MCPServer.Tool createPut_Owners_Owner_IdTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getPut_Owners_Owner_IdHandler(config));
    }
